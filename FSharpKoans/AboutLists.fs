...
module ``about lists`` =
    ...
    let BuildingNewLists() =
        ...
        //                created?
        //Nein, "Property 'Head'/ 'Tail' cannot be set"
    ...
    (* THINK ABOUT IT: In general, what performs better for building lists, 
       :: or @? Why?
       :: -> Aufwand O(1), man erstellt ein Element und setzt den Pointer auf eine Liste
       @ -> Aufwand hängt von der Liste ab, an die etwas angefügt werden soll
            (da man durch die ganze Liste bis zum letzten Element durchiterieren muss). 
